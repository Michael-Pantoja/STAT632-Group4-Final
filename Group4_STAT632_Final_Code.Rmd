...
```{r}
model_step <- stats::step(model1, trace = 0)
```
...
roc_model2 <- roc(NBA$TARGET_5Yrs, predict(model_step, type = "response"))
...
```{r}
# Predict using model1 and calculate accuracy
predicted_lables_model1 <- predict(model1, type = "response") > 0.5
accuracy_model1 <- sum(predicted_lables_model1 == NBA$TARGET_5Yrs) / nrow(NBA)

predicted_lables_model_step <- predict(model_step, type = "response") > 0.5
accuracy_model_step <- sum(predicted_lables_model_step == NBA$TARGET_5Yrs) / nrow(NBA)

cat("Accuracy for model 1: ", accuracy_model1, "\n")
cat("Accuracy for model step: ", accuracy_model_step, "\n")
```
